...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            # Define conditions to detect the "A" gesture
            # Check if the thumb is extended and the rest of the fingers are closed
            if lm_list [4][1] < lm_list [5][1] and lm_list [8][1] > lm_list [6][1] and lm_list [12][1] > lm_list [10][1] and lm_list [16][1] > lm_list [14] [1] and lm_list [20][1] > lm_list [18][1]:
                cv2.putText(frame, "A", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
                print("A")
            # Check if the index finger's x-coordinate is to the left of the middle finger    
            #if lm_list[8][1] < lm_list[12][1]:
                #cv2.putText(frame, "STOP", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
                #print("STOP")
             # Check if the index finger's x-coordinate is to the right of the middle finger    
           # elif lm_list[8][1] > lm_list[12][1]:
               # cv2.putText(frame, "STOP", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
               # print("STOP")     
